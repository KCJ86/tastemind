 ...
 * Summary: Recommendations route — PostgreSQL version
 */
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      // Check daily limit
      const dailyCount = await getDailyRecommendationCount(user.id);
      ...
      await logRecommendation(user.id);

      // Run all 3 Places searches in parallel
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      const user = await getUserByCode(req.body.user_code);
      if (!user) return res.status(404).json({ error: "User not found" });

      const visit = await saveVisit(user.id, req.body);
      res.status(201).json({
        success: true,
